...
@RequestMapping(path = arrayOf("/people"), produces = arrayOf("application/json"))
class PeopleController @Autowired constructor(private val peopleRepository: PeopleRepository) {
    ...
    fun person(@PathVariable id: UUID): Person {
        ...
        val person = peopleRepository.findById(id).orElseThrow { NotFoundException("Person with id=$id was not found.") }
        return person
    }
